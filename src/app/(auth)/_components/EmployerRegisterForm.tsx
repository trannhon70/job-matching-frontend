...
import { employerSendVerifyCodeApi } from "@/services/apis";
...
export function EmployerRegisterForm() {
  ...
  const sendCodeMutation = useMutation({
    mutationFn: (data: { email: string }) => {
      return employerSendVerifyCodeApi(data);
    },
    onSuccess: (res) => {
  ...
  async function onSubmit(values: z.infer<typeof formSchema>) {
    ...
    const res = await signIn("sign-up-employer", {
      email,
      password,
      code,
      callbackUrl: "/",
  ...
  return (
    <div className={cn("grid gap-6")}>
      ...
      <p className="text-center text-sm">
        ...
        <Link className="text-blue-600 underline" href="/employer-login">
          Login
        </Link>
      </p>
    </div>
