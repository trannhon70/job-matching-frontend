...
interface Props {
  id: number;
  type: "education" | "skill" | "certificate" | "exp";
  queryKey: EmployeeQueryKeys;
}

export const DeleteModal: React.FC<Props> = ({ id, type, queryKey }) => {
  const { deleteEducation, deleteSkill, deleteCertificate, deleteExp } =
    useEmployeeApi();
  const {
    value: isOpenModal,
    setTrue: openModal,
  ...
  const deleteMutation = useMutation({
    mutationFn: () => {
      if (type === "education") return deleteEducation(id);
      if (type === "skill") return deleteSkill(id);
      if (type === "certificate") return deleteCertificate(id);
      if (type === "exp") return deleteExp(id);
