...
export const DeleteEmployerModal: React.FC<Props> = ({ employerId }) => {
  const { isFirstMaster } = useCheckAuthorization();
  const { deleteInviteEmployer } = useEmployerApi();
  const {
    value: isOpenModal,
    setTrue: openModal,
    setFalse: closeModal,
  ...
  const deleteMutation = useMutation({
    mutationFn: () => {
      return deleteInviteEmployer(employerId);
    },
    onSuccess: (res) => {
      toast.success(res?.data?.message);
      queryClient.invalidateQueries([EmployerQueryKeys.CONTROL_EMPLOYER]);
      closeModal();
    },
    onError: (err) => {
