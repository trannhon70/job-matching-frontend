...
import { useMount, usePrevious, useSizeCallback } from "@/hooks/common";
import { isShallowEqual } from "@/utils/zoom";
...
const ShareView = forwardRef((props: ShareViewProps, ref: any) => {
  ...
  const [originalViewPosition, setOriginalViewPosition] = useState({
    x: 0,
    y: 0,
  });
  ...
  const onShareViewDrag = useCallback(
    (_event: any, { x, y }: { x: any; y: any }) => {
      ...
      setOriginalViewPosition((payload) => {
        ...
        let nY = payload.y;
        if (
          (x < 0 && Math.abs(x) < width - DragThreshod) ||
          (x > 0 && x < vWidth - DragThreshod)
        ) {
          nX = x;
        }
        if (
          (y < 0 && Math.abs(y) < height - DragThreshod) ||
          (y > 0 && y < vHeight - DragThreshod)
        ) {
          nY = y;
        }
        return {
    ...
    <>
      ...
      >
        ...
        <Draggable
          ...
          disabled={
            isControllingUser || viewType !== ShareViewType.OriginalSize
          }
          position={
            viewType === ShareViewType.OriginalSize
